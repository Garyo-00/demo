...
<div class="spec"><div class="spec-h">📝 詳細仕様</div><div class="spec-body"><ul><li>日付ごとに配置図を管理。台紙セレクタで切替、前回からコピー可（前日に限らず、同じ図面の配置図がある直近の日を辿る）。</li><li>台紙上に作業・資機材・ゲート・注意の配置（マーカー）を表示。</li><li>元請連絡事項（日付ごと）を記入。1日に登録台紙分の配置図を作成可。</li></ul></div></div>
</div>
...
<li>元請がサイドバー「QRコード発行」（単独メニュー）からQRを発行。</li><li>作業予定一覧（ログイン必要）／作業実績入力（不要）／資機材・ゲート予約（不要）の3種類をチェックボックスで選び、選んだぶんを1枚にまとめて印刷。</li>
